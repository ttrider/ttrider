  ...
  if (match) {
    ...
    if (!isNaN(bookNo)) {
      ...
      if (book) {
        ...
        if (elTitle) {
          elTitle.textContent = booklistSource.series
            ? `${booklistSource.bookTitle} ${bookNo} - ${book.title} - ${booklistSource.series} by ${booklistSource.author}`
            : `${book.title} - ${booklistSource.author}`;
        }

        const elHeaderTitle = document.querySelector("div.title");
        if (elHeaderTitle) {
          elHeaderTitle.textContent = booklistSource.series
            ? `${booklistSource.bookTitle} ${bookNo} - ${book.title}`
            : `${booklistSource.bookTitle}`;
        }

        const elHeaderSeries = document.querySelector("a.series");
        if (elHeaderSeries) {
          elHeaderSeries.textContent = booklistSource.series
            ? `${booklistSource.series}`
            : `${booklistSource.bookTitle} - ${booklistSource.author}`;
